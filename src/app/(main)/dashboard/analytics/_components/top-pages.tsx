...
export function TopPages({ data }: { data: any }) {
  ...
  return (
    <Card className="h-full gap-2">
      <CardHeader>
        <CardTitle className="font-normal text-muted-foreground text-sm">Product Page Performance</CardTitle>
      </CardHeader>
                ...
                <TableRow className="hover:bg-muted/10" key={`${page.path}-${index}`}>
                  <TableCell className="max-w-[200px] truncate py-4 font-medium">
                    <div>
                      ...
                      <div className="truncate text-muted-foreground text-xs">{page.name}</div>
                    </div>
                  </TableCell>
                  <TableCell className="text-right tabular-nums font-mono">{page.views}</TableCell>
                  <TableCell className="text-right text-muted-foreground tabular-nums font-mono">{page.time}</TableCell>
                  <TableCell className="text-right text-muted-foreground tabular-nums font-mono">{page.bounce}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
