...
export function TaskReminders({ data }: { data: any }) {
  const billCuts = data?.salesKpis?.billCuts?.current || 379;

  const { proposalSent, proposalGoal, proposalProgressPercentage, proposalGoalBars } = useMemo(() => {
    const sent = Math.round(billCuts * 0.08) || 12;
    const goal = Math.round(sent * 1.5) || 18;
    const progress = Math.round((sent / goal) * 100);
    
    const barCount = 42;
    const activeBars = Math.round((sent / goal) * barCount);
    ...
    return {
      proposalSent: sent,
      proposalGoal: goal,
      proposalProgressPercentage: progress,
      proposalGoalBars: bars,
    };
  }, [billCuts]);
              ...
              <div className="absolute top-2 bottom-2 left-[22%] flex w-[44%] items-center rounded-lg bg-primary px-2 text-primary-foreground shadow-sm">
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="truncate text-[10px] text-primary-foreground/75 font-sans">Weblabs Studio</div>
                  </div>
                </div>
              </div>
      ...
      <Card className="xl:col-span-4">
        ...
        <CardContent className="flex flex-col gap-1">
          <div className="flex items-end justify-between gap-3">
            <div className="font-medium text-2xl tabular-nums leading-none font-bold">
              {proposalSent} <span className="font-normal text-base text-muted-foreground font-sans">sent</span>
            </div>
            <div className="text-muted-foreground text-sm tabular-nums font-semibold">{proposalGoal} target</div>
          </div>
