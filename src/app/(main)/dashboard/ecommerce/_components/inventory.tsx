...
export function Inventory({ data }: { data: any }) {
  ...
  const { inStock, lowStock, outOfStock, availablePercent, gaugeSegments } = useMemo(() => {
    ...
    const inStockSegments = Math.round((inStockVal / totalVal) * gaugeSegmentCount);
    const lowStockSegments = Math.round((lowStockVal / totalVal) * gaugeSegmentCount);
