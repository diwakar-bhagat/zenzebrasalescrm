...
export function RecentOrders({ data }: { data: any }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="font-normal text-muted-foreground text-sm">Recent Transactions</CardTitle>
              ...
              {recentOrders.length > 0 ? (
                recentOrders.map((order: any) => (
                  <TableRow key={order.id}>
                    <TableCell className="font-mono font-medium">{order.billNo}</TableCell>
                    ...
                    <TableCell className="text-right font-semibold font-mono">{formatCurrency(order.netAmount)}</TableCell>
                  </TableRow>
                ))
              ) : (
                <TableRow>
