...
export function StoreTraffic({ data }: { data: any }) {
  ...
  const totalOrders = useMemo(() => {
    return trends.reduce((acc: number, curr: any) => acc + (curr.orders || 0), 0);
  }, [trends]);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="font-normal text-muted-foreground text-sm">Store Transaction Volume</CardTitle>
      ...
      <CardContent>
        <ChartContainer config={trafficConfig} className="h-54 w-full">
          <AreaChart accessibilityLayer data={trends} margin={{ bottom: 0, left: 0, right: 0, top: 8 }}>
            <defs>
