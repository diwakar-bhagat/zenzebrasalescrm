...
export function TrafficSources({ data }: { data: any }) {
	...
	const footfallRows = useMemo(() => {
		const totalBillCuts = storePerformance.reduce(
			(acc: number, row: any) => acc + Number(row.billCuts ?? 0),
			0,
		);
		return storePerformance.map((row: any) => {
			...
			return {
				...
				billCuts,
				share:
					totalBillCuts > 0 ? Math.round((billCuts / totalBillCuts) * 100) : 0,
				growth: Number(row.billCutsGrowth ?? 0),
			};
		});
	}, [storePerformance]);

	const totalVisits = footfallRows.reduce(
		(acc: number, row: any) => acc + row.billCuts,
		0,
	);
			...
			<CardContent>
				<ChartContainer config={trafficSourcesConfig} className="h-54 w-full">
					...
					>
						...
						>
							<LabelList
								content={<TrafficSourceNameLabel footfallRows={footfallRows} />}
								dataKey="name"
							/>
							<LabelList
								content={<FootfallCountLabel footfallRows={footfallRows} />}
								dataKey="share"
							/>
							<LabelList
				...
				<p className="mt-3 text-muted-foreground text-xs">
					Footfall is estimated from distinct bill cuts in the uploaded sales
					sheet.
				</p>
			</CardContent>
		</Card>
