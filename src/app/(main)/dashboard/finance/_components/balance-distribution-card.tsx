...
export function BalanceDistributionCard({ data }: { data: any }) {
  ...
  const chartData = React.useMemo(() => {
    ...
    return storePerformance.map((item: any) => {
      ...
      return {
        ...
        fill: key === "Smart_Works_Noida" ? "var(--chart-4)" : "var(--chart-2)",
      };
    });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="font-normal text-muted-foreground text-sm">Store Allocation Split</CardTitle>
      </CardHeader>
                  ...
                  return (
                    <text dominantBaseline="middle" textAnchor="middle" x={viewBox.cx} y={viewBox.cy}>
                      ...
                      <tspan
                        ...
                        y={(viewBox.cy ?? 0) + 14}
                      >
                        {formatCurrency(currentTotalRevenue, { noDecimals: true })}
                      </tspan>
                    </text>
                  );
                }}
        ...
        <div className="flex min-w-0 flex-col gap-3">
          {chartData.map((item: any) => (
            <div className="grid grid-cols-[1fr_auto] items-end gap-3" key={item.key}>
              ...
              <div className="font-medium tabular-nums font-mono">{item.percentage}%</div>
            </div>
          ))}
        </div>
      </CardContent>
