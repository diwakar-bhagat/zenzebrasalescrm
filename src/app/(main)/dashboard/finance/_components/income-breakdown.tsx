...
export function IncomeBreakdown({ data }: { data: any }) {
	...
	const processedCategories = useMemo(() => {
		const totalCatRevenue = categoryPerformance.reduce(
		...
		const thirdAmount = Math.max(
			0,
			totalCatRevenue - firstAmount - secondAmount,
		);
					...
					<section key={cat.name} className="isolate flex gap-[0.5px]">
						...
						<div className="flex min-h-24 flex-1 flex-col justify-between">
							<div className="flex min-w-0 flex-col gap-1 px-1">
								...
								<div className="text-lg leading-none tracking-tight font-bold font-mono">
									{formatCurrency(cat.amount)}
								</div>
							</div>
							<div
								className={`-ml-0.5 h-5 rounded-sm ${cat.fillClass || "bg-chart-3"}`}
							/>
						</div>
					</section>
				))}
