...
export function OverviewKpis({ data }: { data: any }) {
  ...
  const { netWorth, prevNetWorth, cash, prevCash, spend, prevSpend } = useMemo(() => {
    return {
    ...
    <div className="overflow-hidden rounded-xl bg-card ring-1 ring-foreground/10">
      <div className="grid grid-cols-1 xl:grid-cols-8">
          ...
          <CardContent className="flex items-end justify-between">
            <div className="space-y-1">
              ...
              <p className="text-muted-foreground text-xs">
                {formatCurrency(Math.abs(netWorth - prevNetWorth))} vs last period
              </p>
            </div>
            <Badge className={`border-transparent ${growth >= 0 ? "bg-green-500/10 text-green-700 dark:bg-green-500/15 dark:text-green-300" : "bg-destructive/10 text-destructive"}`}>
        ...
        <Card className="gap-5 overflow-hidden rounded-none border-0 border-foreground/10 border-b ring-0 xl:col-span-4">
          ...
          <CardContent className="flex items-end justify-between">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-muted-foreground text-xs">Derived from 45% sales margin</p>
            </div>
            <Badge className={`border-transparent ${growth >= 0 ? "bg-green-500/10 text-green-700 dark:bg-green-500/15 dark:text-green-300" : "bg-destructive/10 text-destructive"}`}>
              {growth >= 0 ? "+" : ""}{growth.toFixed(1)}%
            </Badge>
        ...
        <Card className="gap-5 overflow-hidden rounded-none border-0 border-foreground/10 ring-0 xl:col-span-4 xl:border-r">
          ...
          <CardContent className="flex items-end justify-between">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-muted-foreground text-xs">Operational & inventory cost (35%)</p>
            </div>
            <Badge className={`border-transparent ${growth >= 0 ? "bg-green-500/10 text-green-700 dark:bg-green-500/15 dark:text-green-300" : "bg-destructive/10 text-destructive"}`}>
              {growth >= 0 ? "+" : ""}{growth.toFixed(1)}%
            </Badge>
        ...
        <Card className="gap-5 overflow-hidden rounded-none border-0 ring-0 xl:col-span-4">
          ...
          <CardContent className="flex items-end justify-between">
            <div className="flex flex-col gap-1">
              <div className="text-3xl leading-none tracking-tight font-bold">26%</div>
              <p className="text-muted-foreground text-xs">Standard operating margin</p>
            </div>
            <Badge className="border-transparent bg-green-500/10 text-green-700 dark:bg-green-500/15 dark:text-green-300">+2.4%</Badge>
          </CardContent>
        </Card>
      </div>
    </div>
