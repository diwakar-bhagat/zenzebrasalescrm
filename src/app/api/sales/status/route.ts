...
export async function GET() {
  try {
    const [statsResult, latestBatchResult, storesResult, categoriesResult, brandsResult, freshnessResult, categoryBrandMapResult] = await Promise.all([
      sql`
    ...
    const categoryBrandMap = (categoryBrandMapResult || []).reduce((acc: Record<string, string[]>, row: any) => {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        availableStores: storesResult.map((row) => row.billed_by).filter(Boolean),
        ...
        dataFreshness: freshnessResult[0]
          ? {
              ...
              daysStale: freshnessResult[0].days_stale != null ? Number(freshnessResult[0].days_stale) : null,
