      ...
      `,
		...
		return NextResponse.json({
			...
			data: {
				...
				categoryBrandMap,
				// Data freshness is no longer derived from upload batches. Sync health now comes from
				// the ERP telemetry endpoint; nothing in the UI ever consumed this field.
			},
		});
	} catch (error: any) {
		if (error?.code === "42P01") {
