...
export async function GET(req: NextRequest) {
	const { requestId, startTime } = logger.startRequest("GET", "/api/v1/commercial/intelligence");
	...
	try {
		...
		return createApiErrorResponse(err.message || "Failed to fetch commercial intelligence", {
			requestId,
			startTime,
		});
	}
