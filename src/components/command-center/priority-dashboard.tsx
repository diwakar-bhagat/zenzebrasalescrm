		...
		useQuery<CommandCenterResponse>({
			...
			queryFn: async () => {
				const url = severityFilter
					? `/api/command-center?severity=${severityFilter}`
					: "/api/command-center";
			...
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
				<div>
					...
					<p className="text-sm text-muted-foreground mt-1">
						Real-time actionable insights driven by deterministic ERP
						constraints.
					</p>
				</div>
				...
				<div className="flex items-center gap-3 text-sm">
					{meta?.computedAt && (
						<span className="text-muted-foreground flex items-center gap-1">
							...
							Updated{" "}
							{formatDistanceToNow(parseISO(meta.computedAt), {
								addSuffix: true,
							})}
						</span>
					)}
			...
			<div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
				{isLoading && !data ? (
					["sk-1", "sk-2", "sk-3", "sk-4"].map((key) => (
						<Card
							key={key}
							className="animate-pulse bg-muted/20 border-border/50"
						>
					...
					<>
						...
						<KpiCard
							...
							onClick={() =>
								setSeverityFilter(severityFilter === "high" ? null : "high")
							}
						/>
						<KpiCard
							title="Medium"
							...
							onClick={() =>
								setSeverityFilter(severityFilter === "medium" ? null : "medium")
							}
						/>
						<KpiCard
							title="Low"
							...
							onClick={() =>
								setSeverityFilter(severityFilter === "low" ? null : "low")
							}
						/>
					</>
				)}
				...
				<CardHeader className="bg-muted border-b pb-4">
					<div className="flex justify-between items-center">
						<div>
							...
							<CardDescription>
								Attention required ranked by cascade risk and delivery
								deadlines.
							</CardDescription>
						</div>
						{severityFilter && (
								...
								<TableRow>
									<TableCell colSpan={6} className="h-32 text-center">
										<p className="text-muted-foreground">
											No active priority signals detected.
										</p>
									</TableCell>
								</TableRow>
							) : (
