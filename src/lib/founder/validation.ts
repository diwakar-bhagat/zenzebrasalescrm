...
function parseTime(value: unknown): string | null {
  ...
  let [, hoursStr, minutesStr, secondsStr, meridiem] = timeMatch;
...
function parseSaleDate(value: unknown): string | null {
  ...
  const dateWithoutTime = text.replace(/\s*\d{1,2}:\d{2}(?::\d{2})?(?:\s*(?:AM|PM|am|pm))?\s*$/, "").trim();

  if (!dateWithoutTime) return null; // If only time was provided, invalid

  // ✅ Format 3a: YYYY/MM/DD (2026/06/15) - YOUR FORMAT!
  const yyyySlashMatch = dateWithoutTime.match(/^(\d{4})\/(\d{1,2})\/(\d{1,2})$/);
  if (yyyySlashMatch) {
  ...
  const ddDashMatch = dateWithoutTime.match(/^(\d{1,2})[-\/](\d{1,2})[-\/](\d{4})$/);
  if (ddDashMatch) {
...
export function validateCanonicalSheet(rows: Record<string, unknown>[]): LegacyValidationResult {
  ...
  for (let i = 0; i < normalizedRows.length; i++) {
    ...
    if (!saleDate) {
      rowErrors.push(`Invalid sale_date: "${row.sale_date}". Expected formats: YYYY/MM/DD (2026/06/15), DD-MM-YYYY (15-06-2026)`);
    }
    ...
    if (row.sale_time) {
      ...
      if (!saleTime && row.sale_time) {
        console.warn(`⚠️ Row ${rowNumber}: Could not parse time: "${row.sale_time}"`);
      }
    }
    ...
    if (!Number.isInteger(quantity) || quantity <= 0) {
      if (Number.isNaN(quantity)) {
        rowErrors.push(`Invalid quantity: "${row.quantity}". Expected a number > 0`);
      }
      quantity = 1;
    }
    ...
    if (!Number.isFinite(netAmount) || netAmount < 0) {
      if (!Number.isFinite(netAmount)) {
        rowErrors.push(`Invalid net_amount: "${row.net_amount}". Expected a number`);
      }
      netAmount = 0;
    }
