...
export const METRIC_REGISTRY: Record<string, MetricDefinition> = {
	...
	AOV: {
		...
		owner: "Executive",
		formula: "SUM(net_amount) / NULLIF(COUNT(DISTINCT bill_no), 0)",
		sqlSource: "sales_fact_v (net_amount, bill_no)",
	...
	TOTAL_BILLS: {
		...
		owner: "Operations",
		formula: "COUNT(DISTINCT bill_no)",
		sqlSource: "sales_fact_v.bill_no",
		calculationFile: "src/lib/business-logic/sales.ts",
		apiRoute: "/api/sales/dashboard",
