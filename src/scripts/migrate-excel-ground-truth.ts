...
async function migrate() {
  ...
  for (const [from, to] of renames) {
    ...
    const targetExists = await sql`
      ...
      LIMIT 1
    `;
    if (Array.isArray(exists) && exists.length > 0 && Array.isArray(targetExists) && targetExists.length === 0) {
      await sql.query(`ALTER TABLE sales_fact RENAME COLUMN ${from} TO ${to}`);
      console.log(`  Renamed sales_fact.${from} → ${to}`);
    }
  }

  for (const col of ["store", "sale_time", "total_amount", "sgst", "cgst", "igst", "row_number"]) {
    const exists = await sql`
      SELECT 1 FROM information_schema.columns
  ...
  await sql`ALTER TABLE sales_fact ADD COLUMN IF NOT EXISTS discount_amount NUMERIC`;
  ...
  try {
    await sql`
      ...
      UNIQUE (sale_date, bill_no, billed_by, sku_code)
    `;
  } catch (error) {
  ...
  await sql`
    ...
    SELECT
      sf.*,
      CASE sf.billed_by
        WHEN 'SmartworksNoida Noida' THEN 'Smart Works Noida'
        WHEN 'Klj store' THEN 'KLJ'
      END AS store_display_name
    FROM sales_fact sf
    WHERE sf.billed_by IN ('SmartworksNoida Noida', 'Klj store')
  `;
